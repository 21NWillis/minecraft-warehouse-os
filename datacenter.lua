...
-- All offsets are datum-frame: the turtle's start cell on the portal is
-- (0,0,0), so the campus ground plane (level with the portal block) is y=-1.
M.SITES = {
  { key = "pad", name = "Portal Plaza", at = { -8, -1, -8 },
    gen = function() return schematic.pad(17, 17, P, { holes = { { 8, 8 } } }) end,
    after = { "the portal sits in the center hole - do not cover it",
              "stage material chests + the NOC-side ender chest here" } },
  { key = "noc", name = "NOC / control room", at = { -4, -1, 10 },
    gen = function() return schematic.hall(9, 7, 9, P, { door = "-z" }) end,
    after = { "advanced computer + ender modem inside; run `update` then `menu`",
  ...
  { key = "warehouse", name = "Warehouse hall", at = { -28, -1, -6 },
    ...
    after = { "storage controller + barrels/chests on a wired modem network",
              "warehouse computer by the door; crafter turtle pool on the west wall" } },
  ...
  { key = "casino", name = "Strainer casino deck", at = { -7, -1, -30 },
    ...
    after = { "FIRST verify strainer behavior (planning/strainer_floor.md)",
              "then: water in the channels, strainers in rows, collection under" } },
}
